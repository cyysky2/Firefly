...
def main():
    ...
    model_name_or_path = 'merge_qlora/firefly-baichuan2-13b-sft-qlora-merge'
    ...
    for question, _ in tqdm(test_data, desc="Processing Questions", unit="question"):
        history = []  # Reset history for each question
        input_ids = build_prompt(tokenizer, template, question, copy.deepcopy(history), system=None).to(model.device)
        ...
        output_data.append([question, response])
